...
一套可定制、可维护的 agentic 中文写作系统，用于起草、编辑、改写、润色和审稿，减少常见中文 AI 味。

这不是一段孤立提示词：

- `SKILL.md` 提供永久控制流程和紧凑写作规则。
- `references/patterns-and-examples.md` 保存详细句式族、解释和例子。
...
中文 AI 味有自己的高频问题：公文腔、营销腔、翻译腔、四字标签、总分总模板、三联轻句、短视频口号、假亲密、强行升华和毒性正能量。这个 kit 同时处理语言表面和推理问题：
...
完整运行规则保留在一个 240 行以内的文件中。详细分类、长句式族和例子放在 reference；任何真正决定成稿是否失败的条件仍必须出现在 `SKILL.md`。
...
加载整个文件夹可以使用脚本、reference 和定制功能。只上传 `SKILL.md` 也能作为手动降级方案，但无法运行相对路径脚本。
...
agent 只会在收到明确请求后创建 `skill-customized.md`。新格式定制文件只保存第 1 至第 8 节，不复制永久控制器。已有旧格式定制文件仍可使用，系统不会静默覆盖或迁移它。
...
agent 会搜索精确表达、近似变体和根本问题。已经覆盖时不重复添加；需要补充时，同时更新适用的核心规则、reference、扫描器和测试。
